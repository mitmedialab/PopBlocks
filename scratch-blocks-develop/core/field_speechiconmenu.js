...
goog.provide('Blockly.SpeechIconMenu');

goog.require('Blockly.DropDownDiv');

let speech_names = ["red", "orange", "yellow", "green", "teal", "blue", "purple"];
 ...
 * @constructor
 */
Blockly.SpeechIconMenu = function(icons) {
  /** @type {object} */
  this.speech_icons_ = speech_icons_;
  // Example:
  // [{src: '...', width: 20, height: 20, alt: '...', value: 'machine_value'}, ...]
  // First icon provides the default values.
  var defaultValue = '';
  Blockly.SpeechIconMenu.superClass_.constructor.call(this, defaultValue);
  
  this.addArgType('iconmenu');
};
goog.inherits(Blockly.SpeechIconMenu, Blockly.Field);
 ...
 * @const
 */
Blockly.SpeechIconMenu.DROPDOWN_WIDTH = 168;
 ...
 * @private
 */
Blockly.SpeechIconMenu.savedPrimary_ = null;

/**
 * Called when the field is placed on a block.
 * @param {Block} block The owning block.
 */
Blockly.SpeechIconMenu.prototype.init = function(block) {
  ...
  if (this.value === null || this.value_ === undefined) {
 	this.setParentFieldImage(this.getSrcForValue('bubble'));
  } else {
  ...
  Blockly.SpeechIconMenu.superClass_.init.call(this, block);
};
 ...
 * @const
 */
Blockly.SpeechIconMenu.prototype.CURSOR = 'default';
 ...
 * @override
 */
Blockly.SpeechIconMenu.prototype.setValue = function(newValue) {
  ...
  // Find the relevant icon in this.speech_icons_ to get the image src.
  this.setParentFieldImage(this.getSrcForValue(this.value_));
};
 ...
 * @private
 */
Blockly.SpeechIconMenu.prototype.setParentFieldImage = function(src) {
 ...
 * @private
 */
Blockly.SpeechIconMenu.prototype.setDropdownImage = function(src) {
  //this.arrowIcon_.setAttributeNS('http://www.w3.org/1999/xlink',
  //    'xlink:href', Blockly.mainWorkspace.options.pathToMedia + 'dropdown-arrow.svg');
  // Only attempt if we have a set sourceBlock_
  if (this.sourceBlock_) {
    ...
    for (var i = 0, input; input = sourceBlock.inputList[i]; i++) {
      for (var j = 0, field; field = input.fieldRow[j]; j++) {
        if (field instanceof Blockly.SpeechIconMenu) {
          // Src for a FieldImage is stored in its value.
          if (field.arrowIcon_) {
 ...
 */
Blockly.SpeechIconMenu.prototype.getValue = function() {
  return this.value_;
};
 ...
 */
Blockly.SpeechIconMenu.prototype.getSrcForValue = function(value) {
  return Blockly.mainWorkspace.options.pathToMedia + "icons/speech" + value + ".svg";
};
 ...
 * @private
 */
Blockly.SpeechIconMenu.prototype.showEditor_ = function() {
  ...
  for (var i = 0, icon; icon = speech_icons_[i]; i++) {
    ...
    button.setAttribute('id', ':' + speech_names[i]); // For aria-activedescendant
    ...
    buttonImg.src = Blockly.mainWorkspace.options.pathToMedia + 'icons/speech' + speech_names[i] + '.svg';
 ...
 * @private
 */
Blockly.SpeechIconMenu.prototype.plusClick_ = function(e) {
  var i = speech_icons_.length;
  if (i < 7) {
	  ...
	  var contentDiv = Blockly.DropDownDiv.getContentDiv();
	  	var icon = {value: '' + speech_names[i],
	  				width: 48,
	  				height: 48,
	  				alt: 'Record ' + (i+1)};
	  	speech_icons_.push(icon);
	    var button = document.createElement('button');
	    ...
	    var buttonImg = document.createElement('img');
	    buttonImg.src = Blockly.mainWorkspace.options.pathToMedia + 'icons/speech' + speech_names[i+1] + '.svg';
	    //buttonImg.alt = icon.alt;
	    // Upon click/touch, we will be able to get the clicked element as e.target
	    // Store a data attribute on all possible click targets so we can match it to the icon.
	    button.setAttribute('data-value', icon.value);
 ...
 * @private
 */
Blockly.SpeechIconMenu.prototype.buttonClick_ = function(e) {
 ...
 */
Blockly.SpeechIconMenu.prototype.onHide_ = function() {
